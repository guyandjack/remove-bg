-- WizPix schema (utf8mb4) - Pre-production / PlanetHoster import
-- ------------------------------------------------------------------
-- Objectif:
-- - Importable directement dans une base DÃ‰JÃ€ crÃ©Ã©e (panel PlanetHoster/phpMyAdmin)
-- - Sans `CREATE DATABASE`, sans `USE`, sans bloc destructif (DROP auto)
-- - Sans noms qualifiÃ©s par schÃ©ma (pas de `db`.`table`) pour rester portable
--
-- Notes:
-- - Les noms de tables `User`, `Plan`, `Subscription` peuvent Ãªtre sensibles selon l'environnement.
--   CÃ´tÃ© SQL, on les protÃ¨ge dans les vues/contraintes quand c'est pertinent.
-- - Les "flags" `active` (OTP) et `is_active` (Subscription) utilisent la convention:
--     - `= 1` pour l'entrÃ©e active
--     - `NULL` pour les entrÃ©es historiques/inactives (permet plusieurs lignes inactives)
-- ------------------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS `TokenRefresh` (
  ...
  revoked          TINYINT(1)    NOT NULL DEFAULT 0,        -- 1 si rÃ©voquÃ© (invalidÃ©)
  revoked_at       DATETIME      NULL,                      -- date de rÃ©vocation
  replaced_by_jti  VARCHAR(191)  NULL,                      -- rotation: jti qui remplace celui-ci
  token_hash       VARBINARY(64) NULL,                      -- optionnel: hash (sha-256) du token si stockÃ©
  ip               VARCHAR(45)   NULL,                      -- IPv4/IPv6
  user_agent       VARCHAR(255)  NULL,                      -- UA au moment de l'Ã©mission
...
-- Ledger d'usage de crÃ©dits (fenÃªtre 24h par requÃªte)
...
CREATE TABLE IF NOT EXISTS `EmailVerification` (
  ...
  expires_at     DATETIME      NOT NULL,
  plan_type      VARCHAR(64)   NULL,     -- ex: 'free','hobby','pro' (option: rÃ©fÃ©rencer Plan.code)
  currency_code  CHAR(3)       NOT NULL DEFAULT 'CHF',
  consumed_at    DATETIME      NULL,
  attempts       TINYINT UNSIGNED NOT NULL DEFAULT 0,
  -- Trick for uniq(email, active): keep active=1 only for current OTP, set inactive rows to NULL.
  active         TINYINT(1)    NULL DEFAULT 1, -- 1 = actif, NULL = inactif
  created_at     TIMESTAMP     NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at     TIMESTAMP     NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  account        TINYINT UNSIGNED NOT NULL DEFAULT 0, -- 1 = compte crÃ©Ã©
